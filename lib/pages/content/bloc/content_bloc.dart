...
class ContentBloc extends Bloc<ContentEvent, ContentState> {
    ...
    try {
      ...
      if (event.category != null) {
        if (event.uid == App().sharedPreferences.getString("uid")) {
          listMyContent =
              (await fireStoreUsers.where('uid', isEqualTo: event.uid).get())
                  .docs;
          listMyContent.forEach((item) {
            listContentModel.add(ContentModel.fromJson(item.data()));
          });
          listContentModel = listContentModel
              .where((i) => i.category == event.category)
              .toList();
        } else {
        ...
        listMyContent = (await fireStoreUsers
                .where('uid',
                    isEqualTo: App().sharedPreferences.getString("uid"))
                .get())
            .docs;
        listMyContent.forEach((item) {
          listContentModel.add(ContentModel.fromJson(item.data()));
  ...
  Stream<ContentState> _mapSearchContent(SearchContentEvent event) async* {
    try {
      ...
      listSearchContentModelTemp = listSearchContentModel
          .where((i) => i.title.contains(event.searchText))
          .toList();

      yield SearchContentState(
          listContentSearchResult: listSearchContentModelTemp);
    } catch (e, stackTrace) {
      yield ContentErrorState(e.toString());
    ...
    try {
      ...
      await fireStoreUsers.doc(event.contentModel.idCont).update({
        ...
        'imageUrl': imageUrl != null
            ? imageUrl.toString()
            : event.contentModel.imageUrl,
        'docUrl':
            docUrl != null ? docUrl.toString() : event.contentModel.docUrl,
      });
  ...
  Stream<ContentState> _mapRecommendInDetail(
      RecommendationInDetailEvent event) async* {
    try {
      yield InitialContentState();
      yield LoadingState();
      ...
      listContent = (await fireStoreUsers
              .limit(5)
              .where('category', isEqualTo: event.contentModel.category)
              .get())
          .docs;
      ...
      yield RecommendationInDetailState(
          listRecommContentDetail: listContentModel);
    } catch (e, stackTrace) {
  ...
  Stream<ContentState> _mapCommonData() async* {
    try {
      ...
      listCommon =
          (await fireStoreUsers.where('isCommon', isEqualTo: true).get()).docs;
      listUnusual =
          (await fireStoreUsers.where('isUnusual', isEqualTo: true).get()).docs;
      ...
      yield CommonDataState(
          listCommon: listCommonModel, listUnusual: listUnusualModel);
    } catch (e, stackTrace) {
      yield ContentErrorState("Gagal mendapatkan data");
      await CrashlyticsHelper.crash(e, stackTrace, "recommend detail content");
      await CrashlyticsHelper.setUserIdentifier(
