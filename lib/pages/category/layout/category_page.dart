...
class _CategoryPageState extends State<CategoryPage> {
  ...
  void initState() {
    ...
    _contentBloc.add(ReadContentEvent(
        category: widget.contentModel.category, uid: widget.contentModel.uid));
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        elevation: 0.0,
        title: Text(
          "Kategori " + widget.contentModel.category,
      ...
      body: Container(
          padding: EdgeInsets.all(16), child: _widgetDetailCategory()),
    );
  }
    ...
    return BlocBuilder(
      ...
      builder: (context, state) {
        return state is ReadContentState && state.listContentModel.length > 0
            ? ListView.builder(
                itemCount: state is ReadContentState &&
                        state.listContentModel.length > 0
                    ? state.listContentModel.length
                    : null,
                itemBuilder: (context, int index) {
                  ContentModel contentModel = state is ReadContentState &&
                          state.listContentModel.length > 0
                      ? state.listContentModel[index]
                      : null;
                  return InkWell(
                    onTap: () {
