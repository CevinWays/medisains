...
class AuthBloc extends Bloc<AuthEvent, AuthState> {
  ...
  Stream<AuthState> mapEventToState(AuthEvent event) async* {
    ...
    else if (event is UpdateProfileEvent) yield* _mapUpdateProfile(event);
  }
  ...
  Stream<AuthState> _mapRegister(RegisterEvent event) async* {
    ...
    yield LoadingState();
    try {
      List<DocumentSnapshot> listUserExist =
          (await firestoreUsers.where('email', isEqualTo: event.email).get())
              .docs;
      ...
      if (_listUserModel.length <= 0) {
        ...
        if (_userCredential.user != null) {
          DocumentReference documentReference =
              firestoreUsers.doc(_userCredential.user.uid);
          if (documentReference.id != null) {
            ...
            SharedPrefHelper.saveUserInfo(_userCredential.user,
                email: event.email, name: event.username);
            yield RegisterState();
          } else {
      ...
      if (_listUserModel.length > 0) {
        ...
        if (_userCredential.user != null) {
          _listUserModel.forEach((item) {
            userModel = UserModel(
                email: item.email, uid: item.uid, username: item.username);
          });

          name = userModel.username;

          SharedPrefHelper.saveUserInfo(_userCredential.user,
              email: event.email, name: name);
          yield LoginState();
        } else {
  ...
  Stream<AuthState> _mapLoginGoogle() async* {
    ...
    try {
      ...
      List<DocumentSnapshot> listUserExist = (await firestoreUsers
  ...
  Stream<AuthState> _mapResetPassword(ResetPassEvent event) async* {
    ...
    try {
      ...
      if (validateEmail != "Email tidak boleh kosong" &&
          validateEmail != "masukkan email yang valid") {
